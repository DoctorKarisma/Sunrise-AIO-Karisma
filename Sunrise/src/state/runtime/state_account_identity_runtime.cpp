...
character_item_definition_hash(const CharacterState& character,
    ...
    for (std::size_t index = 0;
         index < character.inventory.count;
         ++index) {
...
[[nodiscard]] bool
account_owns_soid(const AccountState& account,
    ...
    for (std::size_t index = 0;
         index < account.profileItemCount;
         ++index) {
...
[[nodiscard]] bool
next_item_instance_soid(const AccountState& account,
    ...
    for (std::size_t characterIndex = 0;
        ...
        for (std::size_t index = 0;
            ...
            const std::uint64_t instanceSoid =
                character.inventory.values[index].instanceSoid;
...
[[nodiscard]] bool
next_profile_item_instance_soid(const AccountState& account,
                                std::uint64_t& output) noexcept {
    std::uint64_t candidate =
        authored_inventory::kFirstProfileItemInstanceSoid;
...
/**
 * @return True when an account or character identity already owns
 *         the candidate object key.
 */
[[nodiscard]] bool
identity_uses_soid(const AccountState& account,
                   std::uint64_t soid) noexcept {
    if (soid == 0) {
        return false;
    }

    if (account.primarySoid == soid) {
        return true;
    }
    ...
    for (std::size_t index = 0;
        ...
        for (std::size_t itemIndex = 0;
             itemIndex < character.inventory.count;
             ++itemIndex) {
            if (character.inventory.values[itemIndex].instanceSoid ==
                soid) {
                return true;
            }
        }
...
acquisition_level(const CharacterState& character) noexcept {
    std::int32_t level = 0;
    ...
    for (std::size_t index = 0;
        ...
        level =
            (std::max)(level,
                      character.inventory.values[index].level);
    }
...
[[nodiscard]] bool
find_acquired_row(const family4_loadout::ResolvedLoadout& loadout,
    ...
    for (std::size_t index = 0;
         index < loadout.itemCount;
         ++index) {
        const family4_loadout::ResolvedItem& item =
            loadout.items[index];
...
[[nodiscard]] bool
find_unequipped_row(const family4_loadout::ResolvedLoadout& loadout,
                    std::uint64_t instanceSoid,
                    std::uint16_t& inventoryRow,
                    std::uint8_t& equipmentSlot) noexcept {
    bool found = false;
...
[[nodiscard]] bool
loadout_contains(const family4_loadout::ResolvedLoadout& loadout,
                 std::uint64_t instanceSoid) noexcept {
    for (std::size_t index = 0;
         index < loadout.itemCount;
         ++index) {
        if (loadout.items[index].instance.instanceSoid ==
            instanceSoid) {
            return true;
        }
    }
